Shader "XCTS_Experimental-A01/Opaque_V0.0.1"
{
    Properties
    {
        [Header(Base Color)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings
            {
                ...
                float4 positionCSTranslation : TEXCOORD7;
                float4 positionSSTranslation : TEXCOORD8;
            };
            ...
            Varyings vert(Attributes v)
            {
                ...
                positionCS = positionCS + float4(normalize(normalOS), 0) * (0.01 + _RimLightBias * 0.01);
                o.positionCSTranslation = mul(UNITY_MATRIX_P, positionCS);
                o.positionSSTranslation = ComputeScreenPos(o.positionCSTranslation);
            ...
            float4 frag(Varyings i): SV_Target
            {
                ...
                //-------前期准备-------
                float4 baseMap = SAMPLE_TEXTURE2D(_BaseMap, textureSampler1, (i.uv + _BaseMap_ST.zw) * _BaseMap_ST.xy);
                clip(baseMap.a - _Cutoff);
                float3 albedo = baseMap.rgb * _BaseColor;
                float3 diffuse = albedo;

                float4 normalMap = SAMPLE_TEXTURE2D(_BumpMap, textureSampler1, (i.uv + _BumpMap_ST.zw) * _BumpMap_ST.xy);
                ...
                //-------主光源光照计算-------
                float occlusion = SAMPLE_TEXTURE2D(_OcclusionMap, textureSampler1, (i.uv + _OcclusionMap_ST.zw) * _OcclusionMap_ST.xy).r;
                ...
                //-------高光计算-------
                float4 specularMap = SAMPLE_TEXTURE2D(_SpecularMap, textureSampler1, (i.uv + _SpecularMap_ST.zw) * _SpecularMap_ST.xy);
                ...
                float3 environmentCube = SAMPLE_TEXTURECUBE(unity_SpecCube0, textureSampler1, reflectionDirection).rgb;
                ...
                //-------暗部增强计算-------
                float3 shadeEnhancementColor = SAMPLE_TEXTURE2D(_ShadeEnhancementMap, textureSampler1, (i.uv + _ShadeEnhancementMap_ST.zw) * _ShadeEnhancementMap_ST.xy).rgb;
                ...
                //-------边缘光计算-------
                float linear01FresnelRimLightFactor = smoothstep(0.5 - _RimLightSmoothstep * 0.5, 0.5 + _RimLightSmoothstep * 0.5, (1 - saturate(NdotV - _RimLightBias * 0.5)));
                float linear01Depth = Linear01Depth(SAMPLE_TEXTURE2D(_CameraDepthTexture, textureSampler1, i.positionSS.xy / i.positionSS.w).r, _ZBufferParams);
                float linear01DepthTranslation = Linear01Depth(SAMPLE_TEXTURE2D(_CameraDepthTexture, textureSampler1, min(saturate(i.positionSSTranslation.xy / i.positionSSTranslation.w), 0.99)).r, _ZBufferParams);
                float linear01DepthGap = saturate((linear01DepthTranslation - linear01Depth) * 25 * _CameraFarClip);
                ...
                //-------自发光计算-------
                float3 emissionMap = SAMPLE_TEXTURE2D(_EmissionMap, textureSampler1, (i.uv + _EmissionMap_ST.zw) * _EmissionMap_ST.xy).rgb;
